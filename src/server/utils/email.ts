...
if(!SENDGRID_API_KEY) {
    console.warn('[TechCat Facility Management] Logging emails to console because no SendGrid API key is set in the .env file');
}
...
export const sendEmail = async (email: Email) => {
    if(process.env.NODE_ENV !== 'production') {
        console.warn('[Friendly Butler] Hhrrmm, by the way sir, this mail will not actually be sent because we are in development mode');
        console.log('-----');
